...
contract IPFSLoanTape is AccessControl, Initializable {
    //inputs
    //0)dataId  1.dealName 2)month 3)year 4)servicerName 5)modifiedDate 6)mappingData 7)fileHash 8)privateKey
    event uniqueId(string[] id);
    mapping(string => string[]) map; // unique dataId --> all datas
    mapping(string => bool) map1; // dataId --> true / false --> to test if unique  dataId  is present
    string[] uniqueIdArray;

    function saveLoanDataTape(string[][] memory values) public {
        require(
        ...
        for (i = 0; i < values.length; i++) {
            if(map1[values[i][0]] == false) {
                map[values[i][0]] = values[i]; // pushing all datas for unique dataId
    ...
    function updateLoanDataTape(string[][] memory values) public {
        require(
            hasRole(DEFAULT_ADMIN_ROLE, msg.sender),
            "Caller is not an invoker"
        );
    ...
    function getLoanDataTapeByDealNameMonthYearandServicerName(
